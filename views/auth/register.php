...
<div class="w-full min-h-screen flex justify-center items-center">
    <div class="max-w-[1300px] lg:w-10/12 w-full min-h-screen lg:m-5 flex overflow-hidden lg:rounded-xl shadow-xl">
        <form method="POST" action="/register" class="lg:w-1/2 flex justify-center items-center bg-[#FFD700] w-full lg:p-3">
            <div class="sm:w-[450px] w-[90%] bg-[#fff] flex flex-col justify-center rounded-md p-6 shadow-xl gap-y-3">
                <h1 class="font-bold text-center text-[25px]">Register</h1>
                ...
                <input name="phone" value="<?= isset($old['phone']) ? $this->e($old['name']) : '' ?>" required autofocus type="text" placeholder="0123456789" class="<?= isset($errors['phone']) ? 'border-red-500' : '' ?> block text-[#333] border border-[#333] p-2 focus:outline-none focus:border-yellow-500 focus:ring-yellow-500 rounded-lg">
                ...
                <textarea name="address" maxlength="100" class="w-full block text-[#333] border border-[#333] p-2 focus:outline-none focus:border-yellow-500 focus:ring-yellow-500 rounded-lg" placeholder="Your address"></textarea>
